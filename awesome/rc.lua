...
config_dir = ("/home/ivan/.config/awesome/")
...
awful.key({     }, "XF86AudioMute", function() awful.util.spawn("amixer set PCM toggle", false) end),
...
-- {{ Vim-like controls:
    ...
    awful.key({ modkey },            "r",     function ()
      awful.util.spawn("dmenu_run -i -l 8 -p 'Run command:' -nb '" ..
...
clientkeys = awful.util.table.join(
    ...
    awful.key({ modkey,           }, "t",      function (c)
      if c:titlebar_top():geometry()['height'] > 0 then awful.titlebar.hide(c)
      else
        local layout = wibox.layout.align.horizontal()
...
function run_once(prg,arg_string,pname,screen)
	...
	if not arg_string then 
		...
		awful.util.spawn_with_shell("pgrep -f -u $USER -x '" .. pname .. "'; or " .. prg .. " " .. arg_string, screen)
	end
end
